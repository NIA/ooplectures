...
%----------------------- титульный лист ------------------------
  ...
  \thispagestyle{empty}
  \maketitle
  \todo{упоминание о том, какие базовые знания по языку требуются для этой лекции}
  ...
  \begin{extrainfo}
    Неожиданной особенностью статических полей в Ruby является то, что из значение является общим
  ...
  \begin{halfpage}
    \begin{lstlisting}[language=Java, title={Конструктор в Java}, gobble=6, texcl]
      class Person {
        public Person() {
  ...
  \end{halfpage}
  \begin{halfpage}
    \begin{lstlisting}[language=Ruby, title={Конструктор в Ruby}, gobble=6, texcl]
      class Person
        def initialize()
  ...
  интерпретатором, если не будет объявлен другой.
